...
// DATA SAVER: Only load jobs list WITHOUT heavy images first, then lazy
const load=async()=>{
setLoading(true);
const {data:j}=await supa.from("jobs").select("*").order("id",{ascending:false}).limit(20);if(j)setJobs(j);
const {data:a}=await supa.from("artisans").select("*").order("id",{ascending:false}).limit(20);if(a)setArts(a);
const {data:m}=await supa.from("messages").select("*").order("id",{ascending:true}).limit(100);if(m)setMsgs(m);
const {data:p}=await supa.from("payments").select("*").order("id",{ascending:false}).limit(50);if(p)setPays(p);
const {data:h}=await supa.from("hires").select("*").order("id",{ascending:false}).limit(50);if(h)setHires(h);
const {data:ad}=await supa.from("ads").select("*").order("id",{ascending:false}).limit(10);if(ad)setAds(ad);
setLoading(false);
};

useEffect(()=>{load();},[]); // NO AUTO RELOAD - Saves data!
...
// SUPER COMPRESS - Data saver!
const compressImage=(base64,maxWidth=400,quality=0.4)=>{return new Promise((resolve)=>{const img=new Image();img.onload=()=>{const canvas=document.createElement("canvas");let w=img.width,h=img.height;if(w>maxWidth){h=h*maxWidth/w;w=maxWidth;}canvas.width=w;canvas.height=h;const ctx=canvas.getContext("2d");ctx.drawImage(img,0,0,w,h);resolve(canvas.toDataURL("image/jpeg",quality));};img.src=base64;});};

const up=async(e)=>{const f=e.target.files[0];if(!f)return;if(f.size>10000000){alert("Too big! Use smaller");return;}const r=new FileReader();r.onload=async(v)=>{const c=await compressImage(v.target.result,600,0.4);setJi(c);};r.readAsDataURL(f);};
const upArt=async(e)=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=async(v)=>{const c=await compressImage(v.target.result,300,0.4);setAport(c);};r.readAsDataURL(f);};
const upWorks=(e)=>{const files=Array.from(e.target.files).slice(0,3);files.forEach(f=>{const r=new FileReader();r.onload=async(v)=>{const c=await compressImage(v.target.result,400,0.4);setAworks(prev=>[...prev,c].slice(0,3));};r.readAsDataURL(f);});}; // Only 3 photos now!
const upAd=async(e)=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=async(v)=>{const c=await compressImage(v.target.result,500,0.5);setAdImg(c);};r.readAsDataURL(f);};
...
if(!aport){alert("Upload profile");return;}
try{
...
alert(`✅ ${an} created! Data saved: ${(JSON.stringify(aworks).length/1000).toFixed(0)}KB only!`);
...
const hireArtisan=async(artisanEmail,artisanName,jobId)=>{const jid=jobId||chatJob?.id||jobs[0]?.id;if(!jid)return alert("Select job");const {error}=await supa.from("hires").insert([{job_id:jid,client_email:user.email,artisan_email:artisanEmail,artisan_name:artisanName,status:"hired"}]);if(error){alert(error.message);return;}alert(`Hired ${artisanName}!`);load();setSelectedArt(null);};
const sendMsg=async()=>{if(!chatTxt.trim()||!chatJob)return;const {error}=await supa.from("messages").insert([{job_id:chatJob.id,sender:user.email,receiver:"all",message:chatTxt}]);if(error){alert(error.message);return;}setChatTxt("");const {data:m}=await supa.from("messages").select("*").order("id",{ascending:true}).limit(100);if(m)setMsgs(m);};
...
<small style={{fontSize:10,color:"#155724"}}>🌿 Data Saver Mode ON - 95% less data!</small>
...
{jobs.map(j=>{const b=getBudget(j.budget);const hired=isHired(j.id);return(<div key={j.id} style={{background:"#fff",margin:"10px 15px",borderRadius:12,overflow:"hidden",border:"1px solid #e5e7eb"}}>{j.image_url&&<div style={{position:"relative"}} onClick={()=>setPv(j.image_url)}><img loading="lazy" src={j.image_url} style={{width:"100%",height:160,objectFit:"cover"}} alt="job"/><div style={{position:"absolute",top:8,right:8,display:"flex",gap:"6px"}}>{hired&&<span style={{background:"#0A1931",color:"#FFD700",padding:"4px 8px",borderRadius:20,fontSize:9,fontWeight:"bold"}}>Hired: {hired.artisan_name}</span>}{user?.email===j.created_by&&<button onClick={(e)=>{e.stopPropagation();delJob(j.id);}} style={{background:"#fee2e2",color:"#ef4444",border:"none",borderRadius:"50%",width:28,height:28}}>🗑️</button>}</div></div>}<div style={{padding:"10px"}}><b style={{fontSize:13}}>{j.title}</b><br/><small>📍 {j.location} • 💰 ₦{b.toLocaleString()}</small><br/><small style={{color:"#555",fontSize:11}}>{j.description}</small><div style={{display:"flex",gap:6,marginTop:8}}><button onClick={()=>{if(!user)return alert("Login");setChatJob(j);}} style={{flex:1,padding:"9px",border:"none",borderRadius:8,background:"#0A1931",color:"#FFD700",fontWeight:"bold",fontSize:11}}>💬 Chat</button>{!hasPaid(j.id,"5%_client")&&<button onClick={()=>openPay(j,"client")} style={{flex:1,padding:"9px",border:"none",borderRadius:8,background:"#FFD700",color:"#0A1931",fontWeight:"bold",fontSize:11}}>Unlock</button>}</div></div></div>)})}
</div>}

{tab==="artisans"&&<div style={{padding:"12px"}}>
<h3>Artisans • Portfolio - Data Saver</h3>
...
{filteredArts.map(a=>{const works=parseWorks(a.works);return(
...
{tab==="join"&&<div style={{padding:"12px"}}><h3>Join as Artisan - Lite Upload</h3><div style={{background:"#fff",padding:"12px",borderRadius:12}}>
...
<div style={{marginTop:"8px"}}><b style={{fontSize:11}}>Profile Photo (Compressed to 30KB)</b><input type="file" accept="image/*" onChange={upArt} style={{marginTop:"4px",marginBottom:"8px"}}/>{aport&&<img src={aport} style={{width:60,height:60,borderRadius:"50%",border:"2px solid #FFD700",objectFit:"cover"}} alt="port"/>}</div>
<div style={{marginTop:"10px",background:"#f5f7fb",padding:"8px",borderRadius:10,border:"1px dashed #0A1931"}}>
<b style={{fontSize:11}}>📸 Jobs Done (Max 3 photos - Data Saver!)</b>
...
{tab==="chats"&&isAdmin&&<div style={{padding:"12px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><h2>👁️ All Chats ({msgs.length})</h2><button onClick={load} style={{padding:"6px 10px",background:"#0A1931",color:"#FFD700",border:"none",borderRadius:8,fontSize:10}}>🔄 Refresh</button></div><div style={{background:"#fff",padding:"10px",borderRadius:12,marginTop:"10px"}}>{msgs.length===0&&<div style={{textAlign:"center",padding:"20px",color:"#666"}}>No chats yet</div>}{msgs.map(m=><div key={m.id} style={{borderBottom:"1px solid #eee",padding:"8px 0",fontSize:11}}><b>{m.sender?.split("@")[0]}</b> Job {m.job_id}: {m.message}</div>)}</div></div>}

{selectedArt&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"#fff",zIndex:70,overflowY:"auto"}}><div style={{background:"#0A1931",color:"#fff",padding:"12px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0}}><div style={{display:"flex",alignItems:"center",gap:"8px"}}><img src={selectedArt.portfolio} style={{width:40,height:40,borderRadius:"50%",border:"2px solid #FFD700",objectFit:"cover"}} alt="art"/><div><b style={{color:"#FFD700"}}>{selectedArt.name}</b><div style={{fontSize:10}}>{selectedArt.skill} • {selectedArt.location}</div></div></div><button onClick={()=>setSelectedArt(null)} style={{background:"#fff",border:"none",borderRadius:"50%",width:32,height:32}}>X</button></div><div style={{padding:"12px"}}><img loading="lazy" src={selectedArt.portfolio} style={{width:"100%",height:220,objectFit:"cover",borderRadius:12,border:"3px solid #FFD700"}} alt="portfolio"/><div style={{background:"#f5f7fb",padding:"10px",borderRadius:10,marginTop:"10px"}}><b style={{fontSize:12}}>About</b><p style={{fontSize:12,marginTop:"4px",color:"#555"}}>{selectedArt.bio}</p></div><div style={{marginTop:"12px"}}><b>📸 Jobs Done ({parseWorks(selectedArt.works).length})</b><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px",marginTop:"8px"}}>{parseWorks(selectedArt.works).map((w,i)=><img key={i} loading="lazy" src={w} onClick={()=>setPv(w)} style={{width:"100%",height:90,objectFit:"cover",borderRadius:10,border:"2px solid #FFD700",cursor:"pointer"}} alt="work"/> )}</div></div><div style={{display:"flex",gap:"8px",marginTop:"14px",position:"sticky",bottom:10,background:"#fff",padding:"8px",borderRadius:10,border:"1px solid #e5e7eb"}}><button onClick={()=>{if(jobs.length>0){setChatJob(jobs[0]);setSelectedArt(null);}}} style={{flex:1,padding:"12px",background:"#0A1931",color:"#FFD700",border:"none",borderRadius:10,fontWeight:"bold"}}>💬 Chat</button><button onClick={()=>hireArtisan(selectedArt.created_by,selectedArt.name)} style={{flex:1,padding:"12px",background:"#FFD700",color:"#0A1931",border:"none",borderRadius:10,fontWeight:"bold"}}>Hire Now</button></div></div></div>}

{chatJob&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"#f5f7fb",zIndex:50,display:"flex",flexDirection:"column"}}><div style={{background:"#0A1931",color:"#fff",padding:"12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><b style={{color:"#FFD700"}}>Chat: {chatJob.title}</b><div style={{fontSize:10}}>Data Saver - Manual refresh</div></div><button onClick={()=>setChatJob(null)} style={{background:"#fff",border:"none",borderRadius:"50%",width:32,height:32}}>X</button></div><div style={{flex:1,overflowY:"auto",padding:"10px"}}>{msgs.filter(m=>m.job_id===chatJob.id).length===0&&<div style={{textAlign:"center",marginTop:"40px",color:"#999"}}><div style={{fontSize:40}}>💬</div><b>No messages yet</b></div>}{msgs.filter(m=>m.job_id===chatJob.id).map(m=><div key={m.id} style={{background:m.sender===user.email?"#0A1931":"#fff",color:m.sender===user.email?"#FFD700":"#000",padding:"8px 12px",borderRadius:"12px",margin:"6px 0",maxWidth:"85%",marginLeft:m.sender===user.email?"auto":"0",fontSize:12,border:"1px solid #e5e7eb"}}><div style={{fontSize:9,opacity:0.7}}>{m.sender?.split("@")[0]}</div><div style={{marginTop:"4px"}}>{m.message}</div></div>))}<div ref={chatEndRef}></div></div><div style={{padding:"10px",background:"#fff",display:"flex",gap:"6px",borderTop:"1px solid #e5e7eb"}}><input value={chatTxt} onChange={e=>setChatTxt(e.target.value)} placeholder="Negotiate..." style={{flex:1,padding:"12px",borderRadius:20,border:"1px solid #ddd"}}/><button onClick={sendMsg} style={{padding:"10px 18px",background:"#0A1931",color:"#FFD700",border:"none",borderRadius:20,fontWeight:"bold"}}>Send</button></div></div>}

{payModal&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.6)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",padding:"15px"}}><div style={{background:"#fff",borderRadius:16,width:"100%",maxWidth:"360px",overflow:"hidden"}}><div style={{background:"#0A1931",color:"#fff",padding:"14px"}}><b style={{color:"#FFD700"}}>Secure Checkout • Paystack</b></div><div style={{padding:"14px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px"}}><span>Budget</span><b>₦{payModal.budget.toLocaleString()}</b></div><div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px",color:"#666",fontSize:12}}><span>Fee ({payModal.type==="client"?"5%":"10%"})</span><span>₦{payModal.fee.toLocaleString()}</span></div><div style={{display:"flex",justifyContent:"space-between",fontWeight:"bold",fontSize:16}}><span>Total</span><span>₦{payModal.type==="client"?payModal.total.toLocaleString():payModal.fee.toLocaleString()}</span></div><button onClick={payWithPaystack} style={{width:"100%",marginTop:"12px",padding:"12px",background:"#0A1931",color:"#FFD700",border:"none",borderRadius:10,fontWeight:"bold"}}>Pay with Paystack</button><button onClick={()=>setPayModal(null)} style={{width:"100%",marginTop:"8px",padding:"10px",background:"#fff",color:"#0A1931",border:"1px solid #ddd",borderRadius:10}}>Cancel</button></div></div></div>}
