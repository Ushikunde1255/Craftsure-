...
const uploadToStorage = async (bucket, b64) => {
  ...
  try{
    ...
    const {error} = await supa.storage.from(bucket).upload(name, blob, {contentType:"image/jpeg", upsert:true});
...
const postJob=async()=>{
  if(!user)return alert("Login first"); if(!jt||!jl)return alert("Fill title & location");
  setUpLoading(true);
  try{
    let imgUrl = ji;
    if(ji) imgUrl = await uploadToStorage("job-images", ji);
    await supa.from("jobs").insert([{title:jt,location:jl,budget:jb,description:jd,image_url:imgUrl,created_by:user.email}]);
...
setUpLoading(true);
try{
  const portfolioUrl = await uploadToStorage("artisan-portfolio", aport);
  const workUrls = [];
  for(const w of aworks){ const url = await uploadToStorage("artisan-works", w); workUrls.push(url); }
  const payload={name:an,skill:askill,location:aloc,whatsapp:awhat||verE,portfolio:portfolioUrl,bio:"Verified - "+verM+" - "+payoutMethod+" "+accNum,works:JSON.stringify(workUrls),rating:4.9,jobs_done:workUrls.length,verified:true,created_by:verE||awhat||user?.email||"guest",phone_verified:pVer,email_verified:eVer,verification_method:verM,payout_method:payoutMethod,bank_name:bankName,account_number:accNum,account_name:accName||an};
  const {error}=await supa.from("artisans").insert([payload]); if(error) throw error;
...
const postAd=async()=>{
  if(!adC||!adT)return alert("Fill company & title");
  setUpLoading(true);
  try{
    const amt=adP==="Basic"?20000:adP==="Premium"?50000:100000;
    let imgUrl = adImg;
    if(adImg) imgUrl = await uploadToStorage("ad-images", adImg);
    const {error}=await supa.from("ads").insert([{company_name:adC,title:adT,image_url:imgUrl,link:adL,package:adP,amount:amt,created_by:user?.email||"guest",status:"active"}]);
    if(error) throw error;
    alert("✅ Ad Posted - Will show on Home page!"); setAdC("");setAdT("");setAdImg("");setAdL(""); load(); setTab("home");
  }catch(e){alert(e.message)} finally{setUpLoading(false)}
};
...
<b style={{fontSize:11}}>ARTISAN UPLOAD PROOF 35% 75% 100%:</b>
